...
function GV_Setup_Map() {
	...
	if (typeof(gv_options.zoom_control) != 'undefined' && (gv_options.zoom_control == 'none' || gv_options.zoom_control === false)) {
		...
		// MAKE A CUSTOM ZC THAT'S SMALL AND SIMPLE?
		gmap.setOptions({panControl:(gvg.mobile_browser?false:true),panControlOptions:{position:google.maps.ControlPosition.TOP_LEFT}});
		if (gv_options.zoom_control && gv_options.zoom_control == 'small') {
			gmap.setOptions({zoomControl:true,zoomControlOptions:{style:google.maps.ZoomControlStyle.SMALL,position:google.maps.ControlPosition.TOP_LEFT}});
		} else {
			gmap.setOptions({zoomControl:true,zoomControlOptions:{style:google.maps.ZoomControlStyle.LARGE,position:google.maps.ControlPosition.TOP_LEFT}});
		}
		if (gv_options.recenter_button !== false && (!gmap.panControlOptions.position || gmap.panControlOptions.position == google.maps.ControlPosition.TOP_LEFT)) {
			var rc_div = document.createElement('div'); rc_div.id = 'gv_recenter_map';
			var op = '50';
			rc_div.title = 'double-click to re-center map';
			rc_div.style.cssText = 'top:37px; left:37px; width:13px; height:13px; position:relative; display:block; cursor:pointer; overflow:hidden;';
	...
	if (gv_options.map_opacity_control) {
	...
	if (gv_options.tracklist_options && (gv_options.tracklist_options.tracklist || gv_options.tracklist_options.enabled)) {
		...
		if (tlo.id == 'gv_tracklist') {
			...
			if ($(tlo.id)) {
				...
				/* // commented out because it'll be done in GV_Finish_Tracklist
				if (!$('gv_tracklist_header') && tlo.header) {
	...
	if (gv_options.center_coordinates !== false || gv_options.measurement_tools !== false) {
		...
		if (!$('gv_center_container')) {
			...
			if (gv_options.measurement_tools !== false) {
				center_html += '<td><div id="gv_measurement_icon" style="display:block; width:23px; height:15px; margin-left:3px; cursor:pointer;"><a href="javascript:void(0);" onclick="GV_Place_Measurement_Tools(); GV_Measurements.New(\'distance\');" title="Click here for measurement tools" style="cursor:pointer;"><img src="'+gvg.icon_directory+'ruler.gif" width="23" height="13" border="0" vspace="1" class="gmnoprint" /></a></div></td>';
			}
		...
		if (gv_options.center_coordinates !== false) {
			if (!$('gv_crosshair_container')) {
				...
				crosshair_inner_div.innerHTML = '<img src="'+gvg.icon_directory+'crosshair.png" alt="" width="15" height="15" />';
	...
	if (gv_options.measurement_tools && gv_options.measurement_tools.visible) {
		GV_Place_Measurement_Tools(gv_options.measurement_tools);
	}
	...
	*/
			...
			if (gv_options.synthesize_fields[f] === true || gv_options.synthesize_fields[f] === false) {
				...
				template = template.toString().replace(gvg.synthesize_fields_pattern,
					function (complete_match,field_name) {
						if (field_name.match(/^lat/i)) { field_name = 'lat'; } // v3??? (put this into a "normalize field name" function?)
						else if (field_name.match(/^(lon|lng)/i)) { field_name = 'lon'; } // v3???
	...
	if (mi.label || mi.label_id) { // draw a permanent label
		...
		if (label_text != '') {
			...
			var label_style = (mi.label_color) ? 'background:#ffffff; border-color:'+mi.label_color+'; color:'+mi.label_color+';' : '';
	...
	marker.gvi.name = (mi.name) ? mi.name : gvg.name_of_unnamed_marker;
...
//GV_Debug("marker.gvi.name = "+marker.gvi.name+", marker.gvi.index = "+marker.gvi.index);
	...
	if (mi.track_number && trk[mi.track_number]) {
		...
		if (trk[mi.track_number].info && trk[mi.track_number].info.bounds) { trk[mi.track_number].info.bounds.extend(marker.position); }
...
//GV_Debug ("* "+GV_Debug_Function_Name(arguments.callee)+" (called from "+GV_Debug_Function_Name(arguments.callee.caller)+")");
	if (gvg.marker_list_exists) {
		...
		if (gvg.marker_list_folders) {
			var fcount = 0;
			var minus_graphic = gvg.icon_directory+'minus.gif';
			var plus_graphic = gvg.icon_directory+'plus.gif';
			var folder_triangle_open = gvg.icon_directory+'folder_triangle_open.gif';
			var folder_triangle_closed = gvg.icon_directory+'folder_triangle_closed.gif';
...
function GV_Folder_Collapse_Toggle(index,force_collapse) {
	...
	var open_graphic = gvg.icon_directory+'folder_triangle_open.gif';
	var closed_graphic = gvg.icon_directory+'folder_triangle_closed.gif';
	if (!gvg.marker_list_folder_state[fname]) { gvg.marker_list_folder_state[fname] = {}; }
...
function GV_Marker_List_Item(m,marker_name) { // marker_name is something like "wpts[1]"
	...
	var name = '<div title="'+gvg.marker_list_text_tooltip+'" '+mouseover+mouseout+'onclick="'+text_click+'" class="gv_marker_list_item_name" style="'+color_style+';">'+m.gvi.name + thumbnail + '</div>';
	name = (mlo.url_links && m.gvi.url) ? '<a '+target+' href="'+m.gvi.url+'" title="'+m.gvi.url+'">'+name+'</a>' : name;
	var d = (m.gvi.shortdesc) ? m.gvi.shortdesc : m.gvi.desc;
	var desc = (mlo.desc && d && d != '-') ? '<div class="gv_marker_list_item_desc" style="white-space:normal; '+color_style+'">'+d+'</div>' : '';
...
function GV_Make_Track_Mouseoverable(trk_index) {
	...
	if ((gv_options.track_tooltips === true || trk[trk_index].info.tooltip === true) && trk[trk_index].info.name) {
		for (var i=0; i<trk[trk_index].overlays.length; i++) {
			var track_part = trk[trk_index].overlays[i];
			track_part.mouseover_listener = google.maps.event.addListener(track_part, "mouseover", function(){ GV_Create_Track_Tooltip(trk[trk_index].info); });
												...
												for (var k=0; k<pm['StyleMap']['Pair'].length; k++) {
													if (pm['StyleMap']['Pair'][k]['key'] && pm['StyleMap']['Pair'][k]['key'] == 'normal' && pm['StyleMap']['Pair'][k]['Style']) {
														...
														if (st['IconStyle']) {
															...
															if (ist['Icon'] && ist['Icon']['href']) { mi.icon = ist['Icon']['href']; }
															if (ist['scale']) { mi.scale = parseFloat(ist['scale']); }
															if (ist['color'] && ist['color'].length == 8) {
																mi.color = '#'+ist['color'].replace(/\w\w(\w\w)(\w\w)(\w\w)/,'$3$2$1');
																mi.opacity = parseInt(ist['color'].replace(/(\w\w)\w\w\w\w\w\w/,'$1'),16)/255;
			...
			for (var i=marker_start_index; i<marker_end_index; i++) {
				...
				for (var tag in row) {
					if (tag.indexOf(tag_prefix) == 0) {
						...
						if (tag == 'link' && row['link']['href']) { value = row['link']['href']; } // Garmin's <link> tag uses an href attribute to hold the URL
...
function GV_Recenter_Per_URL(opts) {
	...
	if (window.location.toString().match(zoom_pattern)) {
		...
		if (zoom_match && zoom_match[1]) { // the appropriate variable was found in the URL's query string
			...
			if (z.match(/[0-9]/)) {
				z = parseFloat(z); if (z > 20) { z = 20; } if (z < 0) { z = 0; }
				new_zoom = z;
			}
		}
	}
	...
	for (var b=0; b<gvg.background_maps.length; b++) {
		...
		if (!mapinfo.url) {
			...
			for (var u=0; u<mapinfo.url_count; u++) {
				...
				var max_zoom = (mapinfo.max_zoom) ? mapinfo.max_zoom : ((mapinfo.max_res) ? mapinfo.max_res : 20);
				var map_properties = {
				...
				gvg.bg[mapinfo.id] = mapinfo.id; // just in cases
			}
		}
...
function GV_Place_Draggable_Box(id,position,draggable,collapsible) {
	...
	if ($(container_id) && position && position.length >= 3) {
		...
		if ($(table_id) && $(handle_id) && (draggable || collapsible)) {
			...
			GV_Place_Div(container_id,x,y,anchor);
			if ($(handle_id)) { $(handle_id).parentNode.removeChild($(handle_id)); }
		}
...
function GV_Build_And_Place_Draggable_Box(opts) {
	...
	if ($(id) && $(container_id) && $(table_id) && $(handle_id)) { // ALL the parts exist already
		...
		if ($(container_id)) { $(container_id).parentNode.removeChild($(container_id)); }
		if ($(table_id)) { $(table_id).parentNode.removeChild($(table_id)); }
		if ($(handle_id)) { $(handle_id).parentNode.removeChild($(handle_id)); }
...
function GV_MapTypeControl() {
	...
	gvg.maptype_control = new GV_Control(mtc_div,'TOP_RIGHT',{left:0,right:5,top:6,bottom:6},1);
}
...
function GV_MapOpacityControl(opacity) {
	...
	var selector = document.createElement("select");
	selector.id = 'gv_opacity_selector';
	...
	gvg.opacitycontrol = new GV_Control(oc_div,'TOP_RIGHT',{left:0,right:5,top:6,bottom:6},2);
...
function GV_Background_Opacity(opacity) {
	...
	if (self.gv_opacity_screen_object && gv_opacity_screen_object.getMap()) { // it already exists, don't delete it
		gv_opacity_screen_object.draw(screen_opacity);
	} else {
		gv_opacity_screen_object = new GV_Opacity_Screen(screen_opacity);
	}
	
	if ($('gv_opacity_selector') && $('gv_opacity_selector').length && $('gv_opacity_selector').options) {
		var si = null;
...
function GV_Setup_Opacity_Screen() {
	...
	GV_Opacity_Screen.prototype.onAdd = function() {
		...
		screen.className = 'gmnoprint gv_opacity_screen';
...
function GV_Place_Measurement_Tools(opts) {
	var op = (opts) ? opts : ((gv_options.measurement_tools && typeof(gv_options.measurement_tools) == 'object') ? gv_options.measurement_tools : []);
	...
	if (op.area) {
		html += '<tr valign="top"><td style="padding-right:4px;"><img src="'+gvg.icon_directory+'measure_area.png" align="absmiddle" width="16" height="16" alt=""></td><td style="font-family:Arial;font-weight:bold;" nowrap>Measure area</td></tr>';
	...
	html += '<img src="http://maps.gstatic.com/mapfiles/iw_close.gif" width="12" height="12" border="0" style="cursor:pointer;" onclick="GV_Remove_Measurement_Tools(); $(\'gv_measurement_icon\').style.display = \'block\';" title="cancel measurement and close this panel" />';
	html += '</td></tr></table>';
	html += '</div>';
...
GV_Measurements = new function() {
	...
	this.Calculate = function(key) {
		...
		if (this.Overlay[key] && this.Overlay[key].getPath().getLength() > 1) {
			if (key == 'distance') {
				var meters = 0;
				for(i=1; i<this.Overlay[key].getPath().getLength(); i++) {
...
function GV_Create_Track_Tooltip(info) {
	// adapted from http://www.econym.demon.co.uk/googlemaps/tooltips4.htm
	if (!gvg.track_tooltip_object || !info || (!info.bounds && !info.center) || !info.name || !gvg.overlay.getProjection) { return false; }
	...
	var origin_x = gmap.getDiv().offsetLeft; var origin_y = gmap.getDiv().offsetTop;
	var lat = (info.center && info.center.lat()) ? info.center.lat() : info.bounds.getCenter().lat();
	var lon = (info.center && info.center.lng()) ? info.center.lng() : info.bounds.getCenter().lng();
	var offset = (gvg.overlay.getProjection) ? gvg.overlay.getProjection().fromLatLngToContainerPixel(new google.maps.LatLng(lat,lon)) : new google.maps.Point(-400,-400);
	var width = gvg.track_tooltip_object.clientWidth;
	var height = gvg.track_tooltip_object.clientHeight;
...
function getBoundsZoomLevel(bounds,size) {
	...
	var MARGIN_FACTOR = 0.7; // Not sure why this is necessary, but it doesn't work well without this
	if (!size) {
...
function GV_Background_Map_List() {
	return [	
		...
		,{ id:google.maps.MapTypeId.TERRAIN, menu_order:1.4, menu_name:'Google terrain', description:'Google terrain map', min_zoom:0, max_zoom:15, bounds:[-180,-90,180,90], bounds_subtract:[] }
		,{ id:'OPENSTREETMAP', menu_order:2.1, menu_name:'OpenStreetMap', description:'OpenStreetMap.org', credit:'Map data from <a target="_blank" href="http://www.openstreetmap.org/copyright">OpenStreetMap.org</a>', error_message:'OpenStreetMap tiles unavailable', min_zoom:1, max_zoom:19, bounds:[-180,-90,180,90], bounds_subtract:[], url:'http://tile.openstreetmap.org/{Z}/{X}/{Y}.png' }
		,{ id:'OPENSTREETMAP_OSMARENDER', menu_order:2.11*0, menu_name:'OSM Osmarender', description:'OpenStreetMap Osmarender', credit:'Map data from <a target="_blank" href="http://www.openstreetmap.org/">OpenStreetMap.org</a>', error_message:'OpenStreetMap tiles unavailable', min_zoom:1, max_zoom:19, bounds:[-180,-90,180,90], bounds_subtract:[], url:'http://tah.openstreetmap.org/Tiles/tile/{Z}/{X}/{Y}.png' }
		...
		,{ id:'YAHOO_AERIAL', menu_order:41.2, menu_name:'Yahoo aerial', description:'Yahoo aerial/satellite imagery', credit:'Aerial imagery from <a target="_blank" href="http://maps.yahoo.com/">Yahoo!</a>', error_message:'Yahoo tiles unavailable', min_zoom:1, max_zoom:17, bounds:[-180,-90,180,90], bounds_subtract:[], url:['http://aerial.maps.yimg.com/ximg?t=a&v=1.7&s=256&x={X}&y={Y}&z={Z}'], tile_function:'function(xy,z){return "http://aerial.maps.yimg.com/ximg?t=a&v=1.7&s=256&x="+xy.x+"&y="+(((1<<z)>>1)-1-xy.y)+"&z="+(18-z);}' }
		,{ id:'YAHOO_HYBRID', menu_order:41.3, menu_name:'Yahoo hybrid', description:'Yahoo "hybrid" map', credit:'Map and aerial imagery from <a target="_blank" href="http://maps.yahoo.com/">Yahoo!</a>', error_message:'Yahoo tiles unavailable', min_zoom:1, max_zoom:17, bounds:[-180,-90,180,90], bounds_subtract:[], background:'YAHOO_AERIAL', url:['http://png.maps.yimg.com/png?t=h&v=4.1&s=256&x={X}&y={Y}&z={Z}'], tile_function:'function(xy,z){return "http://png.maps.yimg.com/png?t=h&v=4.1&s=256&x="+xy.x+"&y="+(((1<<z)>>1)-1-xy.y)+"&z="+(18-z);}' }
		,{ id:'YAHOO_MAP_TRANSPARENT', menu_order:41.11*0, menu_name:'Yahoo map (transparent)', description:'Yahoo map, with transparency', credit:'Map data from <a target="_blank" href="http://maps.yahoo.com/">Yahoo!</a>', error_message:'Yahoo tiles unavailable', min_zoom:1, max_zoom:17, bounds:[-180,-90,180,90], bounds_subtract:[], url:['http://png.maps.yimg.com/png?t=h&v=4.1&s=256&x={X}&y={Y}&z={Z}'], tile_function:'function(xy,z){return "http://png.maps.yimg.com/png?t=h&v=4.1&s=256&x="+xy.x+"&y="+(((1<<z)>>1)-1-xy.y)+"&z="+(18-z);}' }
		...
		// ,{ id:'WEATHER_RADAR_US', menu_order:99*0, menu_name:'US NEXRAD radar', description:'NEXRAD radar', credit:'Radar imagery from IAState.edu', error_message:'MESONET imagery unavailable', min_zoom:1, max_zoom:17, country:'us', bounds:[-152,17,-65,65], bounds_subtract:[-129,49.5,-66,72], tile_size:256, background:'MAPQUEST_STREET_WORLD', url:'http://mesonet.agron.iastate.edu/cgi-bin/wms/nexrad/n0r-t.cgi?version=1.1.1&request=GetMap&format=image/png&transparent=true&srs=EPSG:4326&styles=default&layers=nexrad-n0r-wmst' }
		// ,{ id:'NEXRAD_YAHOO', menu_order:0, menu_name:'NEXRAD (w/YM)', description:'NEXRAD radar', credit:'Radar imagery from IAState.edu', error_message:'MESONET imagery unavailable', min_zoom:1, max_zoom:17, country:'us', bounds:[-152,17,-65,65], bounds_subtract:[-129,49.5,-66,72], tile_size:256, background:'YAHOO_MAP', url:'http://mesonet.agron.iastate.edu/cgi-bin/wms/nexrad/n0r-t.cgi?version=1.1.1&request=GetMap&format=image/png&transparent=true&srs=EPSG:4326&styles=default&layers=nexrad-n0r-wmst' }
		// ,{ id:'NEXRAD_YAHOO_AERIAL', menu_order:0, menu_name:'NEXRAD (w/YA)', description:'NEXRAD radar', credit:'Radar imagery from IAState.edu', error_message:'MESONET imagery unavailable', min_zoom:1, max_zoom:17, country:'us', bounds:[-152,17,-65,65], bounds_subtract:[-129,49.5,-66,72], tile_size:256, background:'YAHOO_AERIAL', url:'http://mesonet.agron.iastate.edu/cgi-bin/wms/nexrad/n0r-t.cgi?version=1.1.1&request=GetMap&format=image/png&transparent=true&srs=EPSG:4326&styles=default&layers=nexrad-n0r-wmst' }
		// ,{ id:'NEXRAD_YAHOO_HYBRID', menu_order:0, menu_name:'NEXRAD (w/YH)', description:'NEXRAD radar', credit:'Radar imagery from IAState.edu', error_message:'MESONET imagery unavailable', min_zoom:1, max_zoom:17, country:'us', bounds:[-152,17,-65,65], bounds_subtract:[-129,49.5,-66,72], tile_size:256, background:'YAHOO_HYBRID', url:'http://mesonet.agron.iastate.edu/cgi-bin/wms/nexrad/n0r-t.cgi?version=1.1.1&request=GetMap&format=image/png&transparent=true&srs=EPSG:4326&styles=default&layers=nexrad-n0r-wmst' }
		// ,{ id:'NEXRAD_BLUEMARBLE', menu_order:0, menu_name:'NEXRAD (w/BM)', description:'NEXRAD radar', credit:'Radar imagery from IAState.edu', error_message:'MESONET imagery unavailable', min_zoom:1, max_zoom:17, country:'us', bounds:[-152,17,-65,65], bounds_subtract:[-129,49.5,-66,72], tile_size:256, background:'BLUEMARBLE', url:'http://mesonet.agron.iastate.edu/cgi-bin/wms/nexrad/n0r-t.cgi?version=1.1.1&request=GetMap&format=image/png&transparent=true&srs=EPSG:4326&styles=default&layers=nexrad-n0r-wmst' }
...
function GV_Define_Background_Map_Aliases() { // these aliases should ALWAYS exist and should only be edited, not removed.
	...
	gvg.bg['GV_TOPO'] = gvg.bg['FOURUMAPS_TOPO'];
	gvg.bg['GV_TOPO_US'] = gvg.bg['ARCGIS_TOPO_US'];
	gvg.bg['GV_TOPO_WORLD'] = gvg.bg['FOURUMAPS_TOPO'];
	gvg.bg['GV_TOPO_CA'] = gvg.bg['NRCAN_TOPORAMA'];
	gvg.bg['GV_TOPO_EU'] = gvg.bg['FOURUMAPS_TOPO'];
	gvg.bg['GV_USFS'] = gvg.bg['CALTOPO_USFS_RELIEF'];
	gvg.bg['GV_OCM'] = gvg.bg['OPENCYCLEMAP'];
	gvg.bg['GV_TRANSIT'] = gvg.bg['THUNDERFOREST_TRANSPORT'];
	gvg.bg['GV_AVIATION'] = gvg.bg['VFRMAP'];
	// BACKWARDS COMPATIBILITY:
	...
	gvg.bg['NEXRAD'] = gvg.bg['WEATHER_RADAR_US'];
}
	...
	for (var i=0; i<gvg.background_maps.length; i++) {
		if (gvg.background_maps[i].menu_order > 0) {
			...
			if (div_id) {
				...
				output += '<tr valign="top"><td>'+link_open+gvg.background_maps[i].menu_name+link_close+'</td><td align="center">'+gvg.background_maps[i].menu_order+'</td><td>'+gvg.background_maps[i].id+'</td>';
...
function GV_Define_Styles() {
	...
	document.writeln('			.gv_tracklist { font:11px Arial,sans-serif; line-height:12px; background:#FFFFFF; text-align:left; }');
	...
	document.writeln('			.gv_marker_list { font:10px Verdana,sans-serif; background:#ffffff; text-align:left; }');
	...
	document.writeln('			.gv_infobox { font:11px Arial,sans-serif; background:#ffffff; text-align:left; border:solid #666666 1px; padding:4px; }');
