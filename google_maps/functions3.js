...
function GV_Setup_Global_Variables() {
	...
	gvg.icons['pin'] = { is:[15,26],ia:[7.5,25],ss:[30,26],iwa:[7,1],isa:[12,16],im:[5,25, 5,15, 2,13, 1,12, 0,10, 0,5, 1,2, 2,1, 4,0, 10,0, 12,1, 13,2, 14,4, 14,10, 13,12, 12,13, 9,15, 9,25, 5,25 ],letters:true };
	gvg.icons['square'] = { is:[11,11],ia:[5,5],ss:[13,13],iwa:[10,2],isa:[5,9],im:[0,0, 10,0, 10,10, 0,10, 0,0],letters:true };
	gvg.icons['triangle'] = { is:[13,13],ia:[6.5,6.5],ss:[15,15],iwa:[11,3],isa:[6,10],im:[0,11, 6,0, 12,11, 0,11],letters:false };
	gvg.icons['diamond'] = { is:[13,13],ia:[6.5,6.5],ss:[13,13],iwa:[11,3],isa:[6,10],im:[6,0, 12,6, 6,12, 0,6, 6,0],letters:false };
	gvg.icons['star'] = { is:[19,19],ia:[9.5,10],ss:[19,19],iwa:[12,5],isa:[13,15],im:[9,1, 17,7, 14,17, 4,17, 1,7, 9,1],letters:false };
	gvg.icons['cross'] = { is:[13,13],ia:[6.5,6.5],ss:[15,15],iwa:[11,3],isa:[6,10],im:[4,0, 8,0, 8,4, 12,4, 12,8, 8,8, 8,12, 4,12, 4,8, 0,8, 0,4, 4,4, 4,0],letters:false };
	gvg.icons['airport'] = { is:[17,17],ia:[8.5,8.5],ss:[19,19],iwa:[13,3],isa:[13,17],im:[6,0, 10,0, 16,6, 16,10, 10,16, 6,16, 0,10, 0,6, 6,0],letters:false };
	...
	gvg.icons['camera'] = { is:[17,13],ia:[8.5,6.5],ss:[19,15],iwa:[13,3],isa:[13,10],im:[1,3, 6,1, 10,1, 15,3, 15,11, 1,11, 1,3],letters:false };
	gvg.icons['tickmark'] = { is:[13,13],ia:[6.5,6.5],iwa:[11,3],isa:[],im:[6,0, 12,6, 6,12, 0,6, 6,0],letters:false };
...
//alert('making a vector icon: '+icon+', '+color);
	if (gvg.icons[icon].vector) {
		color = color.toLowerCase(); if (!gvg.named_html_colors[color] && !color.match(/^#/)) { color = '#'+color; }
		var colored_icon = gvg.icons[icon].vector.replace(/="white"/g,'="'+color+'"').replace(/stop-color:white\b/g,'stop-color:'+color);
...
function GV_Setup_Map() {
	...
	if (typeof(gv_options.zoom_control) != 'undefined' && (gv_options.zoom_control == 'none' || gv_options.zoom_control === false)) {
		...
		if (gv_options.zoom_control == 'large' || gv_options.zoom_control == 'small') { // large custom control
			var zc_div = GV_ZoomControl(gv_options.zoom_control);
	...
	if (gv_options.map_opacity_control && gv_options.map_opacity_control != 'false' && gv_options.map_opacity_control != 'none' && (gv_options.map_opacity_control == 'separate' || gv_options.map_opacity_control == 'map' || gv_options.map_opacity_control == 'both')) {
	...
	gvg.measurement_tools_map = true;
	if (gv_options.measurement_tools && (gv_options.measurement_tools === false || gv_options.measurement_tools == 'none' || gv_options.measurement_tools == 'utilities')) { gvg.measurement_tools_map = false; }
	gvg.measurement_tools_utilities = true;
	if (gv_options.measurement_tools && (gv_options.measurement_tools === false || gv_options.measurement_tools == 'none' || gv_options.measurement_tools == 'map')) { gvg.measurement_tools_utilities = false; }
	if (gv_options.center_coordinates !== false || gvg.measurement_tools_map) {
		...
		if (gv_options.center_coordinates !== false) {
			if (!$('gv_crosshair_container')) {
				...
				crosshair_inner_div.innerHTML = '<img src="'+gvg.embedded_images['crosshair']+'" alt="" width="15" height="15" />';
	...
	if ((gvg.measurement_tools_map || gvg.measurement_tools_utilities) && ((gv_options.measurement_tools && gv_options.measurement_tools.visible) || (gv_options.measurement_options && gv_options.measurement_options.visible))) {
	...
	*/
	...
	if ((mi.icon && mi.icon.toString().match(/([\.\/]|^\s*(none|^no.?icon)\s*$)/i)) || (gvg.garmin_icons && gvg.garmin_icons[mi.icon])) {
		...
		if (i.indexOf('/') < 0) { // it's a custom icon, but still a GPSV standard icon
			...
			var rotation_text = (mi.icon == 'tickmark' && mi.rotation !== null && typeof(mi.rotation) != 'undefined') ? '-r'+( 1000+( 5*Math.round(((parseFloat(mi.rotation)+360) % 360)/5)) ).toString().substring(1,4) : '';
			// BUILD THE IMAGE URL:
			tempIcon.icon.url = (gv_options.vector_markers && gvg.icons[i].vector) ? GV_Vector_Icon(i,color,mi.rotation) : base_url+'/'+color.toLowerCase()+rotation_text+'.'+gvg.icon_suffix;
		}
		tempIcon.icon.gv_offset = new google.maps.Point(x_offset,y_offset);
		mi.icon = i;
	}
...
function GV_MapTypeControl(placement) {
...
function GV_Utilities_Menu(show) {
	if (show !== false) {
		var include_opacity = (gv_options.map_opacity_control !== false && gv_options.map_opacity_control != 'map') ? true : false;
		var include_maptype = (gv_options.map_type_control && gv_options.map_type_control.placement && (gv_options.map_type_control.placement == 'utilities' || gv_options.map_type_control.placement == 'both')) ? true : false;
		...
		var first_heading = (include_maptype || include_opacity) ? 'MAP OPTIONS' : 'UTILITIES';
		var html = '';
		...
		if (include_opacity) {
			...
			html += '</div>';
//			html += '<select id="gv_opacity_selector2" style="font:11px Verdana; background-color:#ffffff">';
//			for (var j=10; j>=0; j--) {
...
//			html += '</select></div>';
		...
		if (include_maptype && $('gv_utilities_menu_maptype')) {
			var mtc_div = GV_MapTypeControl('utilities');
			mtc_div.style.display = 'inline-block';
			$('gv_utilities_menu_maptype').appendChild(mtc_div);
		}
		if (include_opacity && $('gv_utilities_menu_opacity')) {
...
function GV_Define_Garmin_Icons(icon_dir,garmin_icon_set) {
	var garmin_codes = new Array(
		...
		,'Scales','Scenic Area','School','Seaplane Base','Shipwreck','Shopping Center','Short Tower','Shower','Ski Resort','Skiing Area','Skull and Crossbones','Soft Field','Square, Blue','Square, Green','Square, Red','Stadium','Stump','Summit','Swimming Area','Small Game'
...
function GV_Define_Vector_Icons() {
	gvg.icons['airport'].vector = '<svg version="1.0" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="17px" height="17px" viewBox="0 0 17 17"><polygon opacity="1" fill="white" points="11,17 17,11 17,6 11,0 6,0 0,6 0,11 6,17 "/><path opacity="1" fill="#ffffff" stroke="#000000" stroke-width="1" d="M8.5,15.171l-2.614,0.404v-1.921L7.5,12.96V9.39l-6.846,1.176V8.553L7.5,5.821V1.334 c0-0.464,0.56-0.831,1-0.831s1,0.367,1,0.831v4.487l6.846,2.732v2.013L9.5,9.39v3.57l1.614,0.694v1.921L8.5,15.171z"/></svg>';
	gvg.icons['blankcircle'].vector = '<svg version="1.0" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="64px" height="64px" viewBox="0 0 64 64"><circle opacity="1" fill="white" stroke="#000000" stroke-width="1.5" cx="32" cy="32" r="31.25"/></svg>';
	gvg.icons['camera'].vector = '<svg version="1.0" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="17px" height="13px" viewBox="0 0 17 13"><g opacity="1"><rect opacity="1.00" fill="white" stroke="#000000" stroke-width="1" x="1.5" y="3.5" width="14" height="8"/><rect opacity="1.00" fill="white" stroke="#000000" stroke-width="1" x="6.5" y="1.5" width="4" height="2"/><rect opacity="1.00" fill="white" stroke="#000000" stroke-width="0.5" x="2.25" y="2.25" width="2" height="1"/><circle opacity="1.00" fill="white" stroke="#000000" stroke-width="1" cx="8.5" cy="7.5" r="3.5"/></g></svg>';
	gvg.icons['circle'].vector = '<svg version="1.0" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="11px" height="11px" viewBox="0 0 11 11"><circle opacity="1" fill="white" stroke="#000000" stroke-width="1" cx="5.5" cy="5.5" r="5"/></svg>';
	gvg.icons['cross'].vector = '<svg version="1.0" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="13px" height="13px" viewBox="0 0 13 13"><polygon opacity="1" fill="white" stroke="#000000" stroke-width="1" points="4.5,0.5 8.5,0.5 8.5,4.5 12.5,4.5 12.5,8.5 8.5,8.5 8.5,12.5 4.5,12.5 4.5,8.5 0.5,8.5 0.5,4.5 4.5,4.5 "/></svg>';
	gvg.icons['diamond'].vector = '<svg version="1.0" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="13px" height="13px" viewBox="0 0 13 13"><path opacity="1" fill="white" stroke="#000000" stroke-width="1" d="M6.5,0c0,2.364-4.136,6.5-6.5,6.5c2.364,0,6.5,4.137,6.5,6.5 c0-2.363,4.137-6.5,6.5-6.5C10.637,6.5,6.5,2.364,6.5,0z"/></svg>';
	gvg.icons['google'].vector = '<svg version="1.0" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="20px" height="34px" viewBox="0 0 20 34"><g opacity="1"><path opacity="1.00" fill="white" stroke="#000000" stroke-width="1.25" d="M9.5,33.375 C9.5,17.283,0.562,18,0.562,10c0-5.212,4.225-9.438,9.438-9.438S19.438,4.788,19.438,10c0,8-8.938,7.283-8.938,23.375H9.5z"/><circle opacity="1.00" fill="#000000" cx="10" cy="10" r="2.75"/></g></svg>';
	...
	gvg.icons['googlemini'].vector = '<svg version="1.0" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="12px" height="20px" viewBox="0 0 12 20"><path opacity="1" fill="white" stroke="#000000" stroke-width="1.25" d="M5.773,19.5 c0-5.18-1.47-6.978-2.708-8.499C1.676,9.291,0.6,8.559,0.6,5.816C0.6,1.425,4.108,0.7,6,0.7c1.893,0,5.4,0.725,5.4,5.116 c0,2.742-1.076,3.474-2.466,5.184c-1.238,1.522-2.708,3.319-2.708,8.5H5.773z" /></svg>';
	gvg.icons['pin'].vector = '<svg version="1.0" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="15px" height="26px" viewBox="0 0 15 26"><g opacity="1"><linearGradient id="stem_gradient" gradientUnits="userSpaceOnUse" x1="5.4165" y1="19" x2="13.4171" y2="19"><stop offset="0" style="stop-color:white"/><stop offset="1" style="stop-color:#000000"/></linearGradient><path opacity="1.00" fill="url(#stem_gradient)" d="M6,13.5C6,12.672,6.671,12,7.5,12S9,12.672,9,13.5v11C9,25.328,8.329,26,7.5,26S6,25.328,6,24.5 V13.5z"/><radialGradient id="ball_gradient" cx="901.7842" cy="-1134.7051" r="19.9659" gradientTransform="matrix(1 0 0 -1 -895.5 -1128.5)" gradientUnits="userSpaceOnUse"><stop offset="0.20" style="stop-color:white"/><stop offset="0.75" style="stop-color:#000000"/></radialGradient><circle opacity="1.00" fill="url(#ball_gradient)" cx="7.5" cy="7.5" r="7.5"/></g></svg>';
	gvg.icons['square'].vector = '<svg version="1.0" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="11px" height="11px" viewBox="0 0 11 11"><rect opacity="1" fill="white" stroke="#000000" stroke-width="1" x="1.5" y="1.5" width="8" height="8"/></svg>';
	gvg.icons['star'].vector = '<svg version="1.0" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="19px" height="19px" viewBox="0 0 19 19"><polygon opacity="1" fill="white" stroke="#000000" stroke-width="1" points="9.5,1.771 11.361,7.5 17.385,7.5 12.512,11.04 14.374,16.769 9.5,13.229 4.626,16.769 6.488,11.04 1.615,7.5 7.639,7.5 "/></svg>';
	gvg.icons['triangle'].vector = '<svg version="1.0" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="13px" height="13px" viewBox="0 0 13 13"><polygon opacity="1" fill="white" stroke="#000000" stroke-width="1" points="0.866,10.758 6.5,1 12.134,10.758" /></svg>';
	
	gvg.icons['airport'].vector_shadow = 'data:image/svg+xml;base64,';
	...
	gvg.icons['cross'].vector_shadow = 'data:image/svg+xml;base64,';
	gvg.icons['diamond'].vector_shadow = 'data:image/svg+xml;base64,';
	gvg.icons['pin'].vector_shadow = 'data:image/svg+xml;base64,';
	gvg.icons['square'].vector_shadow = 'data:image/svg+xml;base64,';
	gvg.icons['star'].vector_shadow = 'data:image/svg+xml;base64,';
	gvg.icons['tickmark'].vector_shadow = 'data:image/svg+xml;base64,';
	gvg.icons['triangle'].vector_shadow = 'data:image/svg+xml;base64,';
	
	gvg.icons['airport'].shadow = 'data:image/png;base64,';
	gvg.icons['blankcircle'].shadow = 'data:image/png;base64,';
	gvg.icons['camera'].shadow = 'data:image/png;base64,';
	gvg.icons['circle'].shadow = 'data:image/png;base64,';
	gvg.icons['cross'].shadow = 'data:image/png;base64,';
	gvg.icons['diamond'].shadow = 'data:image/png;base64,';
	gvg.icons['google'].shadow = 'data:image/png;base64,';
	gvg.icons['googleblank'].shadow = 'data:image/png;base64,';
	gvg.icons['googlemini'].shadow = 'data:image/png;base64,';
	gvg.icons['pin'].shadow = 'data:image/png;base64,';
	gvg.icons['square'].shadow = 'data:image/png;base64,';
	gvg.icons['star'].shadow = 'data:image/png;base64,';
	gvg.icons['triangle'].shadow = 'data:image/png;base64,';

	// gvg.icons['blankcircle'].shadow = 'data:image/png;base64,';
}
...
function GV_Background_Map_List() {
	return [	
		...
		,{ id:'US_NATIONAL_ATLAS_HYBRID', menu_order:11.4, menu_name:'us: Nat\'l Atlas+aerial', description:'United States National Atlas base map', credit:'Base map from <a target="_blank" href="http://nationalatlas.gov/policies.html">The National Atlas</a>', error_message:'National Atlas unavailable', min_zoom:1, max_zoom:15, country:'us', bounds:[-169,18,-66,72], bounds_subtract:[-129,49.5,-66,72], url:'http://basemap.nationalmap.gov/ArcGIS/rest/services/USGSImageryTopo/MapServer/tile/{Z}/{Y}/{X}' }
		,{ id:'US_COUNTIES', menu_order:11.5, menu_name:'us: County outlines', description:'United States county outlines', credit:'US Counties from <a target="_blank" href="https://tigerweb.geo.census.gov/tigerwebmain/TIGERweb_main.html">US Census Bureau</a>', error_message:'TIGERweb tiles unavailable', min_zoom:5, max_zoom:12, country:'us', bounds:[-169,18,-66,72], bounds_subtract:[-129,49.5,-66,72], tile_size:128, url:['https://tigerweb.geo.census.gov/arcgis/services/TIGERweb/tigerWMS_Current/MapServer/WMSServer?service=WMS&version=1.1.1&request=GetMap&format=image/jpeg&transparent=false&exceptions=application/vnd.ogc.se_inimage&srs=EPSG:4326&styles=&layers=Counties','http://services.nationalmap.gov/arcgis/services/SmallScale1Million/SmallScaleBoundariesWMS/MapServer/WMSServer?service=WMS&request=GetMap&version=1.1.1&request=GetMap&format=image/png&transparent=true&srs=EPSG:4326&layers=15&styles='] }
		...
		,{ id:'AU_NATMAP', menu_order:61.0, menu_name:'au: National Map', description:'Australian National Map', credit:'Maps from <a target="_blank" href="http://www.ga.gov.au/">Geoscience Australia<'+'/a>', error_message:'CalTopo topo tiles unavailable', min_zoom:3, max_zoom:16, country:'au', bounds:[111,-45,160,-9], url:'http://www.ga.gov.au/gisimg/rest/services/topography/National_Map_Colour_Basemap/MapServer/tile/{Z}/{Y}/{X}.png' }
...
function GV_Define_Background_Map_Aliases() { // these aliases should ALWAYS exist and should only be edited, not removed.
	...
	gvg.bg['GV_OSM2'] = gvg.bg['THUNDERFOREST_LANDSCAPE'];
